...
7. 建议：“现在此 ADR 已有状态和依赖项字段，请运行 `/architecture-review` 重新验证覆盖情况。”
...
- **依赖项**：扫描现有 ADR 以查找上游依赖；如果不明确，则假设为 `None`
...
将回答记录在 **ADR 依赖项**章节中。如果没有适用的约束，每个字段均写入 `None`。
